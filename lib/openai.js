...
export async function generateOpenAIContent(prompt, options = {}) {
    ...
    for (let i = 0; i <= maxRetries; i++) {
        try {
            ...
            // Retryable errors: Network errors (no status), 429 (Rate Limit), 500+ (Server Errors)
            const isRetryable = !error.status || error.status === 429 || error.status >= 500;

            if (isRetryable && i < maxRetries) {
                console.warn(`OpenAI API Error (${error.status || error.code || error.message}). Retrying in ${currentDelay}ms... (Attempt ${i + 1}/${maxRetries})`);
