...
//  Defines which modules must be PASSED before a module unlocks.
//  Empty array = always available from the start of that section.
// ════════════════════════════════════════════════════════
export const MODULE_PREREQUISITES = {
  // ── Level 1 ─────────────────────────────────────────
  '1a': [],          // Repeat the Sentence          — always unlocked
  '1b': ['1a'],      // Answer Short Questions        — needs Repeat
  '1c': [],          // Reading: Fill in the Blanks   — always unlocked

  // ── Level 2 ─────────────────────────────────────────
  '2a': ['1b'],      // Respond to a Situation        — needs Short Q
  '2c': [],          // Listening: Write from Dict.   — always unlocked

  // ── Level 3 ─────────────────────────────────────────
  '3a': ['2a'],      // Retell Lecture                — needs Respond
  '3c': ['1c'],      // Reading: Reorder Paragraphs   — needs FITB
  '3d': ['2c'],      // Listening: Highlight Summary  — needs Dictation

  // ── Level 4 ─────────────────────────────────────────
  '4a': ['3a'],      // Evaluating & Comparing        — needs Retell
  '4b': ['3a'],      // Speculating the Future        — needs Retell
  '4c': ['3c'],      // Reading: MC Multiple          — needs Reorder
  '4d': ['3d'],      // Listening: Summarise Text     — needs Highlight

  // ── Level 5 ─────────────────────────────────────────
  '5a': ['4a', '4b'],// Full IELTS Speaking Mock      — needs both L4 speaking
  '5b': ['4a'],      // TOEFL Speaking Challenge      — needs Evaluating
  '5c': ['4c'],      // PTE Reading: Full Section     — needs MC Multiple
  '5d': ['4d'],      // PTE Listening: Full Section   — needs Summarise
};

// ════════════════════════════════════════════════════════
//  XP REWARDS — base XP for first-time module pass per level
//  Bonuses (first-attempt, streak) are added in complete/route.js
// ════════════════════════════════════════════════════════
export const MODULE_XP = {
  1: 50,
  2: 100,
  3: 200,
  4: 350,
...
// ════════════════════════════════════════════════════════
//  PASS THRESHOLDS — minimum normalised score (0-100) to pass
//  For objective tasks  → accuracy %  (correct / total * 100)
//  For subjective tasks → AI confidenceScore (0-100)
// ════════════════════════════════════════════════════════
export const MODULE_PASS_THRESHOLDS = {
  'repeat':    75,   // ≥ 75 % sentence accuracy
  'short':     60,   // ≥ 60 % correct answers (3/5)
  'retell':    65,   // AI confidence ≥ 65
  'respond':   60,   // AI confidence ≥ 60
  'describe':  60,   // AI confidence ≥ 60
  'fitb-r':    70,   // ≥ 70 % correct blanks
  'fitb-rw':   70,
  'fitb-l':    70,
  'reorder':   65,
  'mc-multi':  65,
...
// ════════════════════════════════════════════════════════
//  RANK → DIFFICULTY DESCRIPTOR
//  Used by /api/learning-path/start to calibrate prompt complexity
// ════════════════════════════════════════════════════════
export const RANK_DIFFICULTY = {
  'Newbie': {
    label:        'absolute beginner',
    sentenceWords:'4-7',
    complexity:   'very simple vocabulary, present tense only, everyday objects',
    lectureWords:  50,
    scenarioType: 'basic daily life (ordering food, asking for directions)',
    questionLevel:'basic general knowledge: colors, animals, numbers, simple facts',
  },
  'Beginner': {
    label:        'elementary',
    sentenceWords:'7-12',
    complexity:   'common vocabulary, simple past tense, basic idioms',
    lectureWords:  70,
    scenarioType: 'everyday social situations (booking a taxi, returning an item)',
    questionLevel:'basic GK: world capitals, famous places, simple science',
  },
  'Intermediate': {
    label:        'intermediate',
    sentenceWords:'12-18',
    complexity:   'varied tenses, academic vocabulary mixed with common words, compound sentences',
    lectureWords:  90,
    scenarioType: 'semi-formal contexts (calling to make an appointment, explaining a problem to a manager)',
    questionLevel:'general knowledge: geography, history, science basics',
  },
  'Advanced': {
    label:        'advanced',
    sentenceWords:'18-25',
    complexity:   'complex grammar, idioms, subordinate clauses, rich vocabulary',
    lectureWords:  110,
    scenarioType: 'complex professional or social dilemmas (negotiating a deadline, resolving a conflict)',
    questionLevel:'mixed: literature, history, economics, complex science',
  },
  'Expert': {
    label:        'proficient',
    sentenceWords:'20-30',
    complexity:   'sophisticated language, nuanced meaning, academic register',
    lectureWords:  130,
    scenarioType: 'high-stakes scenarios (presenting to stakeholders, mediating a dispute)',
    questionLevel:'challenging: detailed history, science, culture, specialist knowledge',
  },
  'Master': {
    label:        'native-like',
    sentenceWords:'25-35',
    complexity:   'native-level complexity, varied register, subtle connotation',
    lectureWords:  150,
    scenarioType: 'nuanced real-world situations requiring diplomatic and professional register',
    questionLevel:'expert-level: cross-domain, nuanced, requiring deep understanding',
  },
};
